...
def write_file(path: str, data: bytearray, fixed: int):
    if fixed == 0:
...
def to_le_hex(value: int) -> str:
    return ' '.join(f'{b:02X}' for b in struct.pack('<Q', value))

def scan_segments(data: bytearray) -> int:
    file_size = len(data)
    found = 0
    fixed = 0
    offset = 0
    ...
    while offset <= file_size - STRUCT_SIZE:
        ...
        try:
            header = data[offset:offset + STRUCT_SIZE]
            magic_val, seg_type, stored_crc, size = struct.unpack_from('<IIQI', header)
            ...
            print("=" * 60)
            print(f"{magic_str} @ 0x{offset:08X}")
            print(f"Type          : {seg_type}")
            print(f"Size          : {size}")
            print(f"CRC range     : 0x{segment_offset:08X} - 0x{segment_end:08X}")
            print(f"Stored        : {to_le_hex(stored_crc)}")
            ...
            if seg_type == 1:
                ...
                match = (calc_crc == stored_crc)

                print(f"Calc          : {to_le_hex(calc_crc)}")
            ...
            print(f"Error at 0x{offset:X}: {e}")
    ...
    if found == 0:
        ...
        print(f"\nDone — {found} segment(s) scanned, {fixed} fixed.")

    return fixed

def main():
    if len(sys.argv) != 2:
        print("Usage: python dl2_checksum.py <file>")
        sys.exit(1)

    path = sys.argv[1]
    data = read_file(path)

    fixed = scan_segments(data)

    write_file(path, data, fixed)
